...
export default function ProductPage() {
  ...
  const { list: products, currentPage, totalPages, loading } = useSelector(
  ...
  useEffect(() => {
    dispatch(fetchProducts({ page, limit: 8, search }));
    dispatch(fetchCart());
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 mt-20">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 items-stretch mt-6">
          {products.map((p) => (
            <div
              key={p.id}
